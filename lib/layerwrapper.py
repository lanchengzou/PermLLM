...
class WrappedGPT:
    ...
    def __init__(self, args, layer, layer_id=0, layer_name="none", reconstruct=False):
        ...
        self.inp_mean = torch.zeros((self.columns), device=self.dev) 
        self.inp_square_sum = torch.zeros((self.columns), device=self.dev)
        self.inp_var = torch.zeros((self.columns), device=self.dev)

        self.out_mean = torch.zeros((self.rows), device=self.dev) 
        self.out_square_sum = torch.zeros((self.rows), device=self.dev)
        self.out_var = torch.zeros((self.rows), device=self.dev)
    ...
    def add_batch(self, inp, out):
        ...
        if self.reconstruct or hasattr(self, "H"):
        ...
        inp_mean_batch = torch.abs(inp).mean(dim=1)
        inp_square_sum_batch = (inp ** 2).sum(dim=1)

        self.inp_mean = (self.inp_mean * (self.nsamples - tmp) + inp_mean_batch * tmp) / self.nsamples
        self.inp_square_sum += inp_square_sum_batch
        self.inp_var = self.inp_square_sum / self.nsamples - self.inp_mean ** 2

        out_mean_batch = torch.abs(out).mean(dim=1)
        out_square_sum_batch = (out ** 2).sum(dim=1)

        self.out_mean = (self.out_mean * (self.nsamples - tmp) + out_mean_batch * tmp) / self.nsamples
        self.out_square_sum += out_square_sum_batch
        self.out_var = self.out_square_sum / self.nsamples - self.out_mean ** 2
        # if "up" in self.layer_name or "gate" in self.layer_name:
